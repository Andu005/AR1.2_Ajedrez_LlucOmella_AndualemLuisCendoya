...
void jugar() {
    ...
    while (true) {
        ...
        cout << (turnoBlancas ? "Blancas" : "Negras") << ", ingrese movimiento (ej. a2 a3): ";
        cin >> entrada1 >> entrada2;

        if (entrada1.size() != 2 || entrada2.size() != 2) {
            cout << "Formato incorrecto! Usar ej. a2 a3\n";
            continue;
        }

        int xInicio = 8 - (entrada1[1] - '0');
        int yInicio = tolower(entrada1[0]) - 'a';
        int xFin = 8 - (entrada2[1] - '0');
        int yFin = tolower(entrada2[0]) - 'a';
